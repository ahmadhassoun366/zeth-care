	...
	for (let i = firstPage; i < endPage; i++) {
		...
		pageButtons.push(
			<button
				...
				className={`pagination-btn ${metadata.currentPage === i + 1 && 'active'}`}
			>
				{i + 1}
			</button>
		);
		...
		<div className="pagination">
			...
			<div>
				<h1 className="text-agrey-900 dark:text-white text-sm font-medium">
					...
					{(metadata.currentPage - 1) * metadata.itemsPerPage} -{' '}
					{(metadata.currentPage - 1) * metadata.itemsPerPage + metadata.itemsPerPage} of{' '}
					{metadata.totalItems}
				</h1>
			</div>
			...
			<div className="flex items-center gap-x-2">
				...
				<button
					...
					disabled={
						metadata.currentPage === metadata.totalPages || metadata.totalPages === 0
					}
				>
